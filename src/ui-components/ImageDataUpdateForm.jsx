...
import { getImageData } from "../graphql/queries";
import { updateImageData } from "../graphql/mutations";
...
export default function ImageDataUpdateForm(props) {
  const {
    id: idProp,
    imageData: imageDataModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = imageDataRecord
      ? { ...initialValues, ...imageDataRecord }
      : initialValues;
  ...
  const [imageDataRecord, setImageDataRecord] =
    React.useState(imageDataModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getImageData.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getImageData
        : imageDataModelProp;
      setImageDataRecord(record);
    };
    queryData();
  }, [idProp, imageDataModelProp]);
  React.useEffect(resetStateValues, [imageDataRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateImageData.replaceAll("__typename", ""),
            variables: {
              input: {
                id: imageDataRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ImageDataUpdateForm")}
      {...rest}
    >
      <TextField
        label="Image url"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || imageDataModelProp)}
